<?
/**
 * Переписанный модуль bitrix/modules/main/classes/general/file.php
 * Для ресайза картинок от верха
 *
 * @category
 * @link        http://.ru
 * @revision    $Revision$
...
class File
{
	...
	public static function defineConstants()
	{
		...
		if (!defined('BX_RESIZE_IMAGE_EXACT_TOP')) {
			define('BX_RESIZE_IMAGE_EXACT_TOP', 10);
		}
	...
	public static function ScaleImage($sourceImageWidth, $sourceImageHeight, $arSize, $resizeType, &$bNeedCreatePicture, &$arSourceSize, &$arDestinationSize)
	{
		...
		if ($sourceImageWidth > 0 && $sourceImageHeight > 0) {
			if ($arSize["width"] > 0 && $arSize["height"] > 0) {
				switch ($resizeType) {
					case BX_RESIZE_IMAGE_EXACT_TOP:
	...
	public static function ResizeImageFile($sourceFile, &$destinationFile, $arSize, $resizeType = BX_RESIZE_IMAGE_PROPORTIONAL, $arWaterMark = array(), $jpgQuality = false, $arFilters = false)
	{
		...
		if ($resizeType !== BX_RESIZE_IMAGE_EXACT && $resizeType !== BX_RESIZE_IMAGE_PROPORTIONAL_ALT && $resizeType !== BX_RESIZE_IMAGE_EXACT_TOP)
		...
		if (class_exists("imagick") && function_exists('memory_get_usage')) {
			...
			if ((memory_get_usage() + $memoryNeeded) > $memoryLimit) {
				...
				self::ScaleImage($arSourceFileSizeTmp[0], $arSourceFileSizeTmp[1], $arSize, $resizeType, $bNeedCreatePicture, $arSourceSize, $arDestinationSize);
	...
	/**
	 * Ресайз картинки по ID файла или массиву файла 
	 * 
	 * @param      $file
	 * @param      $arSize
	 * @param      $resizeType
